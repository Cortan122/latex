...
\author{\AA{Костя Борисов}{4} \\ \AA{БПИ199}{11}}
...
\begin{document}
  ...
  \noindent
  \begin{tikzpicture}
    \begin{axis}[xlabel=$\mathcal{A}$,ylabel={$\mathcal{A}$},xmin = -1,xmax = 1,ymin = -1,ymax = 1]
      \addplot[domain=-1:1,samples=200,color=blue] {sin(deg(x*4*pi))/5/pi + x};
    \end{axis}
  \end{tikzpicture} \hfill
  \begin{tikzpicture}
    \begin{axis}[xlabel=$\mathcal{A}$,ylabel={$\mathcal{D}$},xmin = -1,xmax = 1,ymin = -1,ymax = 1]
      \addplot[color=blue] coordinates { (-1,-1) (0,-.5) };
      \addplot[color=blue] coordinates { (0,.5) (1,1) };
      \addplot[holdot] coordinates{(0,-.5)};
      \addplot[soldot] coordinates{(0,.5)};
    \end{axis}
  \end{tikzpicture}
  ...
  \begin{wrapfigure}{r}{4cm}
    ...
    \centering
    \begin{tikzpicture}
      \begin{axis}[xlabel=$\mathcal{B}$,ylabel={$\mathcal{C}$},xmin = -1,xmax = 1,ymin = -1,ymax = 1,width=4cm]
        \addplot[color=blue] coordinates { (-1,-1) (0,-.5) };
        \addplot[color=blue] coordinates { (0,.5) (1,1) };
        \addplot[holdot] coordinates{(0,-.5)(0,.5)};
      \end{axis}
    \end{tikzpicture}
    \vspace{-1cm}
  \end{wrapfigure}
  но для $\mathcal{A} \cong \mathcal{C}$ у нас ничего неполучится.
  та точка где у нас разрыв (на графике это $0$, а тут это будет $r$) не должна быть разрывной в $\mathcal{A}$.
  поэтому нам надо чтобы ровно одна из границ дырки в $\mathcal{Y}$ была закрашена,
  потомучто если закрашены обе или если закрашенных нет, то непонятно чему равно $f(r)$.

  ничего неизменится если мы будем уменьшать радиус дырки (в $\mathcal{C}$ он равен $0.5$).
  если мы будем уменьшать радиус дырки в $\mathcal{C}$, то у нас в конце концов будет $\mathcal{B}$.
  и ничего непоменялось: $\mathcal{B}$ также как и $\mathcal{C}$ неизоморфно $\mathcal{A}$.
  мы даже можем нарисовать график $\mathcal{B} \cong \mathcal{C}$, но у меня кончилось место.
